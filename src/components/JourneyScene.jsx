...
export function JourneyScene() {
  ...
  useFrame((state) => {
    ...
    if (slsGroup.current) {
      ...
      slsGroup.current.position.y +=
        (targetSlsY - slsGroup.current.position.y) * 0.1;
      slsGroup.current.scale.setScalar(targetSlsScale);
    }

    if (orionGroup.current) {
      orionGroup.current.visible = orionVisible;
      orionGroup.current.position.x +=
        (targetOrionX - orionGroup.current.position.x) * 0.1;
      orionGroup.current.position.y +=
        (targetOrionY - orionGroup.current.position.y) * 0.1;
      orionGroup.current.position.z +=
        (targetOrionZ - orionGroup.current.position.z) * 0.1;
      ...
      if (heatShieldRef.current) {
        heatShieldRef.current.opacity +=
          (glowOpacity - heatShieldRef.current.opacity) * 0.04;
        heatShieldRef.current.emissiveIntensity =
          heatShieldRef.current.opacity * 2;
      }
    }
    ...
    earthRef.current.position.x +=
      (targetEarthX - earthRef.current.position.x) * 0.04;
    earthRef.current.position.y +=
      (targetEarthY - earthRef.current.position.y) * 0.04;
    earthRef.current.position.z +=
      (targetEarthZ - earthRef.current.position.z) * 0.04;
  ...
  return (
    <group>
      ...
      <mesh ref={moonRef} visible={false}>
        ...
        <Suspense
          fallback={<meshStandardMaterial color="#cccccc" roughness={0.8} />}
        >
          <TexturedMoon />
        </Suspense>
      </mesh>
